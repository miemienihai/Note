...
创建瓦片地图时，Grid 组件自动作为瓦片地图的父级，并在将瓦片布置到瓦片地图上时作为参照(利用Grid组件当做瓦片的定位器)这里虽然说是定位器,但是Grid 是网格系统,所以相当于Grip就是提供一个个网格.
...
### 1.2 这几个的关系
+ grid 是最上面的父类, 他提供一个个的网格图,方便你往里面填充像素图片, TileMap他类似于一个画板,在网格的辅助下,画图.Tile最终填写到TileMap中去.
...
- Tile ：新版本中已经看不到，但可以使用(一般是你直接把Sprit拖到地图中的那种)
...
(Pivot 一棵树出现在前面的时候是不是树根的位置在人的脚底pivot 前面,这也是为什么pivot 要设置在sprite 最下面的中心)
<br>
...
## 实现图像拉伸sprite 自动复制的特点
+ 在Sprite Renderer 渲染中选择 Draw Mode 为Tiled 图像平铺的特点,同时根据警告解决警告.
+ 注意是在按键T模式下生效.
![](2022-10-02-12-02-04.png)
![](2022-10-02-12-02-13.png)